      ...
      <main className="max-w-[1400px] mx-auto w-full px-4 md:px-10 lg:px-20 py-6 grow">
        ...
        <div className="flex flex-col lg:flex-row gap-10 lg:gap-12 mb-16">
          ...
          <div className="flex flex-col lg:w-1/2">
            ...
            <div className="flex gap-4">
              <div className={`bg-[#F0F0F0] rounded-full flex items-center px-4 py-3 w-1/3 max-w-[150px] justify-between ${product.stock === 0 ? 'opacity-50 pointer-events-none' : ''}`}>
                <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="text-2xl font-medium cursor-pointer focus:outline-none">-</button>
                <span className="font-medium">{product.stock === 0 ? 0 : quantity}</span>
                <button onClick={() => setQuantity(Math.min(product.stock, quantity + 1))} className="text-2xl font-medium cursor-pointer focus:outline-none">+</button>
              </div>
              <button 
                ...
                className={`flex-1 rounded-full font-medium py-3 transition-colors shadow-sm ${
                  isAlreadyInCart || product.stock === 0
                    ? 'bg-gray-300 text-gray-500 cursor-not-allowed' 
                    : 'bg-black text-white hover:bg-gray-800 cursor-pointer'
                }`}
              >
                {product.stock === 0 ? 'Out of Stock' : (isAlreadyInCart ? 'Already in Cart' : (isPreOrder ? 'Pre Order' : 'Add to Cart'))}
              </button>
            </div>
